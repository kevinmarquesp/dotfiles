...
M.misc = function()
    ...
    local function optional_mappings()
        ...
        map("n", "<leader><C-w>",   ":setl wrap! <CR>" )
        map("n", "<leader><Up>",    ":ls <CR>"         )
        ...
        map("n", "<leader>3", ":setl expandtab tabstop=8 shiftwidth=8 softtabstop=8 <CR>")
        ...
        if file:find(".md") ~= nil then
            ...
            local command = string.format(
                ":silent !pandoc %s -o %s; mupdf -r 72 %s & <CR>",
                file, pdf_file, pdf_file
            )

            map("n", "<leader>K", command);
            map("n", "<leader>k", ":MarkdownPreview");
